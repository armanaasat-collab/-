...
<div id="country-modal">
  <div class="country-box">
    <button class="country-close-btn" id="countryModalCloseBtn" aria-label="Жабу">✕</button>
    <h2>Елді таңдаңыз</h2>
  ...
  <div class="page" id="page-transfer">
    ...
    <div class="bank-select-wrap">
      <div class="bank-select-row">
        ...
        <button class="dots-btn" id="bankDotsBtn" aria-label="Банк таңдау" data-i18n-aria="bankSelectAria">
    ...
    <div class="transfer-note" id="transferNote" data-i18n="transferNote">Басқанда сома алмасу буферіне көшіріледі және банк қосымшасы ашылады.</div>
  </div>
...
<div class="overlay-screen" id="settings-modal">
  <div class="overlay-inner">
    <div class="overlay-topbar">
      ...
      <div class="overlay-title" data-i18n="settingsTitle">Баптаулар</div>
    </div>
    <div class="overlay-body">
      <div class="settings-card">
        <div class="settings-card-title" data-i18n="security">Қауіпсіздік</div>
        <div class="settings-row">
          <div>
            <div class="settings-row-label" data-i18n="appLock">Қосымшаны құлыптау</div>
            <div class="settings-row-sub" data-i18n="appLockSub">Ашқан сайын құпия код сұралады</div>
          </div>
          <span class="lock-status-badge off" id="lockStatusBadge" data-i18n="lockOff">Өшірулі</span>
        </div>
        <div id="lockTypeChooser" class="lock-type-grid">
          <button class="lock-type-btn" data-type="pin">
            ...
            <div>
              <div class="lock-type-name" data-i18n="pinName">Сандық код (PIN)</div>
              <div class="lock-type-desc" data-i18n="pinDesc">4 таңбалы сан</div>
            </div>
          </button>
          <button class="lock-type-btn" data-type="pattern">
            ...
            <div>
              <div class="lock-type-name" data-i18n="patternName">Өрнек (Pattern)</div>
              <div class="lock-type-desc" data-i18n="patternDesc">Нүктелерді сызу арқылы</div>
            </div>
          </button>
          <button class="lock-type-btn" data-type="alpha">
            ...
            <div>
              <div class="lock-type-name" data-i18n="alphaName">Әріп + сан</div>
              <div class="lock-type-desc" data-i18n="alphaDesc">Құпия сөз (кемінде 4 таңба)</div>
            </div>
          </button>
        </div>
        <div id="lockManageRow" style="display:none;">
          <button class="settings-plain-btn" id="lockChangeBtn">
            ...
            <span data-i18n="changeLockBtn">Құлыпты өзгерту</span>
          </button>
          <button class="settings-plain-btn danger" id="lockDisableBtn">
            ...
            <span data-i18n="disableLockBtn">Құлыпты өшіру</span>
          </button>
        </div>
      </div>

      <div class="settings-card">
        <div class="settings-card-title" data-i18n="dataSection">Деректер</div>
        ...
        <button class="settings-plain-btn" id="changeLangBtn">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 8h9M9 4v2M13 4v2M7 8c0 5 4 8 4 8M12 8c0 5 4 8 4 8"/><circle cx="12" cy="12" r="10"/></svg>
          <span data-i18n="changeLang">Тілді ауыстыру</span>
        </button>
      </div>
    </div>
  </div>
...
<div class="overlay-screen" id="lock-setup-modal">
  <div class="overlay-inner">
    <div class="overlay-topbar">
      ...
      <div class="overlay-title" data-i18n="lockSetupTitle">Құлыпты орнату</div>
    </div>
...
(function(){
  ...
  const translations = {
    'kk': { itemNamePlaceholder: "Заттың аты", shop: "дүкенге бару", app: "Супер Көмекші", total: "барлығы", list: "Тізім", budget: "Бюджет", stats: "Статистика", transfer: "Аударым", converter: "Конвертер", add: "Тізімге қосу", checkout: "Дүкеннен шықтым — сақтау", continue: "Жалғастыру", share: "Бөлісу", copy: "Көшіру", freq: "Жиі алатын заттар", collected: "жиналды", topSpend: "Ең қымбат заттар", statsAll: "Жалпы көрсеткіштер", avgCheck: "Орташа чек", statsMonths: "Айлар бойынша шығын", statsTop: "Ең көп ақша не үшін кетті", bill: "Есеп", totalAmount: "Жалпы сома", phoneTitle: "Аударатын адамның нөмірі", listEmpty: "Тізім бос. Затты жаз.", saveAlert: "Сақталды!", clearList: "Тізімді тазалау", clearAll: "Барлық деректерді өшіру", clearAllConfirm: "Барлық деректер (тізім, бюджет, тарих, тіл) толығымен өшіріледі. Жалғастырасыз ба?", clearedAllAlert: "Барлық деректер өшірілді", changeLang: "Тілді ауыстыру", exit: "Шығу", exitTitle: "Шығу", exitDesc: "Деректеріңізбен не істейміз?", exitSave: "Шығу — деректерді сақтау", exitDelete: "Шығу — деректерді жою", cancel: "Бас тарту", clearedAlert: "Тізім тазаланды", deletedAlert: "Деректер жойылды",
      settingsTitle: "Баптаулар", security: "Қауіпсіздік", appLock: "Қосымшаны құлыптау", appLockSub: "Ашқан сайын құпия код сұралады", lockOff: "Өшірулі", lockOnSuffix: "қосулы", pinName: "Сандық код (PIN)", pinDesc: "4 таңбалы сан", patternName: "Өрнек (Pattern)", patternDesc: "Нүктелерді сызу арқылы", alphaName: "Әріп + сан", alphaDesc: "Құпия сөз (кемінде 4 таңба)", changeLockBtn: "Құлыпты өзгерту", disableLockBtn: "Құлыпты өшіру", dataSection: "Деректер", lockSetupTitle: "Құлыпты орнату", newSecretTitle: "Жаңа құпияны енгізіңіз", repeatSecretTitle: "Қайталап енгізіңіз", repeatSecretSub: "Растау үшін тағы бір рет енгізіңіз", verifySecretTitle: "Құпияны енгізіңіз", currentLockSub: "Ағымдағы құлыпты енгізіңіз", lockDisabledToast: "Құлып өшірілді", lockSavedToast: "Құлып сақталды", mismatchToast: "Сәйкес келмеді, қайта көріңіз", wrongTryAgainToast: "Қате, қайта көріңіз", patternMinError: "Кемінде 4 нүктені қосыңыз", alphaPlaceholder: "Құпия сөз (әріп + сан)", confirmBtn: "Растау", alphaMinError: "Кемінде 4 таңба керек", wrongCodeToast: "Қате код", appLockedTitle: "Қосымша құлыпталған", appLockedSub: "Кіру үшін құпияны енгізіңіз", forgotLink: "Құпияны ұмыттыңыз ба? Шығу", forgotConfirm: "Барлық деректер (тізім, бюджет, тарих, құлып) өшеді және бастапқы бетке ораласыз. Жалғастыру керек пе?", transferNote: "Басқанда сома алмасу буферіне көшіріледі және банк қосымшасы ашылады.", bankSelectAria: "Банк таңдау", emptyListToast: "Тізім бос — алдымен зат қосыңыз" },
    'ru': { itemNamePlaceholder: "Название товара", shop: "поход в магазин", app: "Супер Помощник", total: "итого", list: "Список", budget: "Бюджет", stats: "Статистика", transfer: "Перевод", converter: "Конвертер", add: "Добавить в список", checkout: "Вышел из магазина — сохранить", continue: "Продолжить", share: "Поделиться", copy: "Копировать", freq: "Частые покупки", collected: "собрано", topSpend: "Самые дорогие", statsAll: "Общие показатели", avgCheck: "Средний чек", statsMonths: "Расход по месяцам", statsTop: "На что ушло больше всего", bill: "Счет", totalAmount: "Общая сумма", phoneTitle: "Номер для перевода", listEmpty: "Список пуст. Добавьте товары.", saveAlert: "Сохранено!", clearList: "Очистить список", clearAll: "Удалить все данные", clearAllConfirm: "Все данные (список, бюджет, история, язык) будут полностью удалены. Продолжить?", clearedAllAlert: "Все данные удалены", changeLang: "Сменить язык", exit: "Выйти", exitTitle: "Выход", exitDesc: "Что сделать с вашими данными?", exitSave: "Выйти — сохранить данные", exitDelete: "Выйти — удалить данные", cancel: "Отмена", clearedAlert: "Список очищен", deletedAlert: "Данные удалены",
      settingsTitle: "Настройки", security: "Безопасность", appLock: "Блокировка приложения", appLockSub: "При каждом открытии будет запрашиваться код", lockOff: "Выключено", lockOnSuffix: "включен", pinName: "Цифровой код (PIN)", pinDesc: "4-значное число", patternName: "Узор (Pattern)", patternDesc: "Соединяя точки", alphaName: "Буквы + цифры", alphaDesc: "Пароль (минимум 4 символа)", changeLockBtn: "Изменить блокировку", disableLockBtn: "Отключить блокировку", dataSection: "Данные", lockSetupTitle: "Настройка блокировки", newSecretTitle: "Введите новый код", repeatSecretTitle: "Повторите ввод", repeatSecretSub: "Введите ещё раз для подтверждения", verifySecretTitle: "Введите код", currentLockSub: "Введите текущий код блокировки", lockDisabledToast: "Блокировка отключена", lockSavedToast: "Блокировка сохранена", mismatchToast: "Не совпадает, попробуйте снова", wrongTryAgainToast: "Ошибка, попробуйте снова", patternMinError: "Соедините минимум 4 точки", alphaPlaceholder: "Пароль (буквы + цифры)", confirmBtn: "Подтвердить", alphaMinError: "Нужно минимум 4 символа", wrongCodeToast: "Неверный код", appLockedTitle: "Приложение заблокировано", appLockedSub: "Введите код для входа", forgotLink: "Забыли код? Выйти", forgotConfirm: "Все данные (список, бюджет, история, блокировка) будут удалены, и вы вернётесь на начальный экран. Продолжить?", transferNote: "При нажатии сумма скопируется в буфер обмена и откроется приложение банка.", bankSelectAria: "Выбор банка", emptyListToast: "Список пуст — сначала добавьте товары" },
    'uz': { itemNamePlaceholder: "Mahsulot nomi", shop: "do'konga borish", app: "Super Yordamchi", total: "jami", list: "Ro'yxat", budget: "Budjet", stats: "Statistika", transfer: "O'tkazma", converter: "Konverter", add: "Ro'yxatga qo'shish", checkout: "Do'kondan chiqdim — saqlash", continue: "Davom etish", share: "Ulashish", copy: "Nusxalash", freq: "Ko'p olinadigan", collected: "yig'ildi", topSpend: "Eng qimmat", statsAll: "Umumiy", avgCheck: "O'rtacha chek", statsMonths: "Oylar bo'yicha", statsTop: "Eng ko'p xarajat", bill: "Hisob", totalAmount: "Umumiy summa", phoneTitle: "O'tkazma raqami", listEmpty: "Ro'yxat bo'sh.", saveAlert: "Saqlandi!", clearList: "Ro'yxatni tozalash", clearAll: "Barcha ma'lumotni o'chirish", clearAllConfirm: "Barcha ma'lumotlar (ro'yxat, byudjet, tarix, til) butunlay o'chiriladi. Davom etasizmi?", clearedAllAlert: "Barcha ma'lumot o'chirildi", changeLang: "Tilni almashtirish", exit: "Chiqish", exitTitle: "Chiqish", exitDesc: "Ma'lumotlaringiz bilan nima qilamiz?", exitSave: "Chiqish — ma'lumotni saqlash", exitDelete: "Chiqish — ma'lumotni o'chirish", cancel: "Bekor qilish", clearedAlert: "Ro'yxat tozalandi", deletedAlert: "Ma'lumotlar o'chirildi",
      settingsTitle: "Sozlamalar", security: "Xavfsizlik", appLock: "Ilovani qulflash", appLockSub: "Har safar ochganda parol so'raladi", lockOff: "O'chirilgan", lockOnSuffix: "yoqilgan", pinName: "Raqamli kod (PIN)", pinDesc: "4 xonali raqam", patternName: "Naqsh (Pattern)", patternDesc: "Nuqtalarni birlashtirish orqali", alphaName: "Harf + raqam", alphaDesc: "Parol (kamida 4 belgi)", changeLockBtn: "Qulfni o'zgartirish", disableLockBtn: "Qulfni o'chirish", dataSection: "Ma'lumotlar", lockSetupTitle: "Qulfni sozlash", newSecretTitle: "Yangi parolni kiriting", repeatSecretTitle: "Qayta kiriting", repeatSecretSub: "Tasdiqlash uchun yana bir marta kiriting", verifySecretTitle: "Parolni kiriting", currentLockSub: "Joriy qulf parolini kiriting", lockDisabledToast: "Qulf o'chirildi", lockSavedToast: "Qulf saqlandi", mismatchToast: "Mos kelmadi, qayta urinib ko'ring", wrongTryAgainToast: "Xato, qayta urinib ko'ring", patternMinError: "Kamida 4 ta nuqtani birlashtiring", alphaPlaceholder: "Parol (harf + raqam)", confirmBtn: "Tasdiqlash", alphaMinError: "Kamida 4 ta belgi kerak", wrongCodeToast: "Noto'g'ri kod", appLockedTitle: "Ilova qulflangan", appLockedSub: "Davom etish uchun parolni kiriting", forgotLink: "Parolni unutdingizmi? Chiqish", forgotConfirm: "Barcha ma'lumotlar (ro'yxat, byudjet, tarix, qulf) o'chiriladi va boshlang'ich ekranga qaytasiz. Davom etasizmi?", transferNote: "Bosganda summa vaqtinchalik xotiraga nusxalanadi va bank ilovasi ochiladi.", bankSelectAria: "Bankni tanlash", emptyListToast: "Ro'yxat bo'sh — avval mahsulot qo'shing" },
    'ky': { itemNamePlaceholder: "Нерсенин аты", shop: "дүкөнгө баруу", app: "Супер Жардамчы", total: "баардыгы", list: "Тизме", budget: "Бюджет", stats: "Статистика", transfer: "Котормо", converter: "Конвертер", add: "Тизмеге кошуу", checkout: "Дүкөндөн чыктым — сактоо", continue: "Улантуу", share: "Бөлүшүү", copy: "Көчүрүү", freq: "Көп алынуучу", collected: "чогултулду", topSpend: "Эң кымбат", statsAll: "Жалпы", avgCheck: "Орточо чек", statsMonths: "Айлар боюнча", statsTop: "Эң көп каражат", bill: "Эсеп", totalAmount: "Жалпы сумма", phoneTitle: "Которуу номери", listEmpty: "Тизме бош.", saveAlert: "Сакталды!", clearList: "Тизмени тазалоо", clearAll: "Бардык дайыектарды өчүрүү", clearAllConfirm: "Бардык дайыектар (тизме, бюджет, тарых, тил) толугу менен өчүрүлөт. Улантасызбы?", clearedAllAlert: "Бардык дайыектар өчүрүлдү", changeLang: "Тилди алмаштыруу", exit: "Чыгуу", exitTitle: "Чыгуу", exitDesc: "Дайыектарыңыз менен эмне кылабыз?", exitSave: "Чыгуу — дайыектарды сактоо", exitDelete: "Чыгуу — дайыектарды өчүрүү", cancel: "Артка", clearedAlert: "Тизме тазаланды", deletedAlert: "Дайыектар өчүрүлдү",
      settingsTitle: "Жөндөөлөр", security: "Коопсуздук", appLock: "Колдонмону кулпулоо", appLockSub: "Ар ачканда код суралат", lockOff: "Өчүрүлгөн", lockOnSuffix: "күйүк", pinName: "Сандык код (PIN)", pinDesc: "4 орундуу сан", patternName: "Үлгү (Pattern)", patternDesc: "Чекиттерди туташтыруу менен", alphaName: "Тамга + сан", alphaDesc: "Сырсөз (кеминде 4 белги)", changeLockBtn: "Кулпуну өзгөртүү", disableLockBtn: "Кулпуну өчүрүү", dataSection: "Дайыектар", lockSetupTitle: "Кулпуну орнотуу", newSecretTitle: "Жаңы сырсөздү киргизиңиз", repeatSecretTitle: "Кайра киргизиңиз", repeatSecretSub: "Ырастоо үчүн дагы бир жолу киргизиңиз", verifySecretTitle: "Сырсөздү киргизиңиз", currentLockSub: "Учурдагы кулпу кодун киргизиңиз", lockDisabledToast: "Кулпу өчүрүлдү", lockSavedToast: "Кулпу сакталды", mismatchToast: "Дал келген жок, кайра аракет кылыңыз", wrongTryAgainToast: "Ката, кайра аракет кылыңыз", patternMinError: "Кеминде 4 чекитти туташтырыңыз", alphaPlaceholder: "Сырсөз (тамга + сан)", confirmBtn: "Ырастоо", alphaMinError: "Кеминде 4 белги керек", wrongCodeToast: "Ката код", appLockedTitle: "Колдонмо кулпуланган", appLockedSub: "Кирүү үчүн сырсөздү киргизиңиз", forgotLink: "Сырсөздү унуттуңузбу? Чыгуу", forgotConfirm: "Бардык дайыектар (тизме, бюджет, тарых, кулпу) өчүрүлөт жана баштапкы бетке кайтасыз. Улантасызбы?", transferNote: "Баскандан сумма алмашуу буферине көчүрүлөт жана банк колдонмосу ачылат.", bankSelectAria: "Банкты тандоо", emptyListToast: "Тизме бош — адегенде зат кошуңуз" },
    'zh': { itemNamePlaceholder: "物品名称", shop: "去商店", app: "超级助手", total: "总计", list: "列表", budget: "预算", stats: "统计", transfer: "转账", converter: "汇率换算", add: "添加到列表", checkout: "离开商店 — 保存", continue: "继续", share: "分享", copy: "复制", freq: "常买物品", collected: "已收集", topSpend: "最贵物品", statsAll: "总览", avgCheck: "平均账单", statsMonths: "按月支出", statsTop: "最大开销", bill: "账单", totalAmount: "总金额", phoneTitle: "转账号码", listEmpty: "列表为空。", saveAlert: "已保存！", clearList: "清空列表", clearAll: "删除所有数据", clearAllConfirm: "所有数据（列表、预算、历史记录、语言）将被完全删除。是否继续？", clearedAllAlert: "所有数据已删除", changeLang: "更改语言", exit: "退出", exitTitle: "退出", exitDesc: "您的数据要如何处理？", exitSave: "退出 — 保存数据", exitDelete: "退出 — 删除数据", cancel: "取消", clearedAlert: "列表已清空", deletedAlert: "数据已删除",
      settingsTitle: "设置", security: "安全", appLock: "应用锁", appLockSub: "每次打开都会要求输入密码", lockOff: "已关闭", lockOnSuffix: "已开启", pinName: "数字密码 (PIN)", pinDesc: "4位数字", patternName: "图案 (Pattern)", patternDesc: "连接圆点", alphaName: "字母 + 数字", alphaDesc: "密码（至少4个字符）", changeLockBtn: "更改锁定", disableLockBtn: "关闭锁定", dataSection: "数据", lockSetupTitle: "设置锁定", newSecretTitle: "输入新密码", repeatSecretTitle: "再次输入", repeatSecretSub: "请再次输入以确认", verifySecretTitle: "输入密码", currentLockSub: "输入当前锁定密码", lockDisabledToast: "锁定已关闭", lockSavedToast: "锁定已保存", mismatchToast: "不匹配，请重试", wrongTryAgainToast: "错误，请重试", patternMinError: "请至少连接4个点", alphaPlaceholder: "密码（字母+数字）", confirmBtn: "确认", alphaMinError: "至少需要4个字符", wrongCodeToast: "密码错误", appLockedTitle: "应用已锁定", appLockedSub: "请输入密码继续", forgotLink: "忘记密码？退出", forgotConfirm: "所有数据（列表、预算、历史记录、锁定）将被删除，并返回初始界面。是否继续？", transferNote: "点击后金额将复制到剪贴板并打开银行应用。", bankSelectAria: "选择银行", emptyListToast: "列表为空—请先添加物品" },
    'tr': { itemNamePlaceholder: "Ürün adı", shop: "alışverişe git", app: "Süper Yardımcı", total: "toplam", list: "Liste", budget: "Bütçe", stats: "İstatistik", transfer: "Transfer", converter: "Çevirici", add: "Listeye ekle", checkout: "Çıktım — kaydet", continue: "Devam et", share: "Paylaş", copy: "Kopyala", freq: "Sık alınanlar", collected: "toplanan", topSpend: "En pahalı", statsAll: "Genel", avgCheck: "Ortalama fiş", statsMonths: "Aylık gider", statsTop: "En çok harcanan", bill: "Hesap", totalAmount: "Toplam", phoneTitle: "Transfer numarası", listEmpty: "Liste boş.", saveAlert: "Kaydedildi!", clearList: "Listeyi temizle", clearAll: "Tüm verileri sil", clearAllConfirm: "Tüm veriler (liste, bütçe, geçmiş, dil) tamamen silinecek. Devam edilsin mi?", clearedAllAlert: "Tüm veriler silindi", changeLang: "Dili değiştir", exit: "Çıkış", exitTitle: "Çıkış", exitDesc: "Verilerinizle ne yapalım?", exitSave: "Çıkış — verileri kaydet", exitDelete: "Çıkış — verileri sil", cancel: "Vazgeç", clearedAlert: "Liste temizlendi", deletedAlert: "Veriler silindi",
      settingsTitle: "Ayarlar", security: "Güvenlik", appLock: "Uygulama kilidi", appLockSub: "Her açtığınızda şifre istenecek", lockOff: "Kapalı", lockOnSuffix: "açık", pinName: "Sayısal kod (PIN)", pinDesc: "4 haneli sayı", patternName: "Desen (Pattern)", patternDesc: "Noktaları birleştirerek", alphaName: "Harf + sayı", alphaDesc: "Şifre (en az 4 karakter)", changeLockBtn: "Kilidi değiştir", disableLockBtn: "Kilidi kapat", dataSection: "Veriler", lockSetupTitle: "Kilit kurulumu", newSecretTitle: "Yeni şifreyi girin", repeatSecretTitle: "Tekrar girin", repeatSecretSub: "Onaylamak için tekrar girin", verifySecretTitle: "Şifreyi girin", currentLockSub: "Mevcut kilit şifrenizi girin", lockDisabledToast: "Kilit kapatıldı", lockSavedToast: "Kilit kaydedildi", mismatchToast: "Eşleşmedi, tekrar deneyin", wrongTryAgainToast: "Hatalı, tekrar deneyin", patternMinError: "En az 4 nokta birleştirin", alphaPlaceholder: "Şifre (harf + sayı)", confirmBtn: "Onayla", alphaMinError: "En az 4 karakter gerekli", wrongCodeToast: "Yanlış kod", appLockedTitle: "Uygulama kilitli", appLockedSub: "Devam etmek için şifreyi girin", forgotLink: "Şifreyi mi unuttunuz? Çıkış", forgotConfirm: "Tüm veriler (liste, bütçe, geçmiş, kilit) silinecek ve başlangıç ekranına döneceksiniz. Devam edilsin mi?", transferNote: "Dokunduğunuzda tutar panoya kopyalanır ve banka uygulaması açılır.", bankSelectAria: "Banka seç", emptyListToast: "Liste boş — önce ürün ekleyin" },
    'en': { itemNamePlaceholder: "Item name", shop: "go to shop", app: "Super Helper", total: "total", list: "List", budget: "Budget", stats: "Stats", transfer: "Transfer", converter: "Converter", add: "Add to list", checkout: "Left shop — save", continue: "Continue", share: "Share", copy: "Copy", freq: "Frequent items", collected: "collected", topSpend: "Top spends", statsAll: "Overall stats", avgCheck: "Avg Check", statsMonths: "Monthly spend", statsTop: "Top categories", bill: "Bill", totalAmount: "Total Amount", phoneTitle: "Transfer Number", listEmpty: "List is empty.", saveAlert: "Saved!", clearList: "Clear list", clearAll: "Delete all data", clearAllConfirm: "All data (list, budget, history, language) will be permanently deleted. Continue?", clearedAllAlert: "All data deleted", changeLang: "Change language", exit: "Exit", exitTitle: "Exit", exitDesc: "What should we do with your data?", exitSave: "Exit — keep data", exitDelete: "Exit — delete data", cancel: "Cancel", clearedAlert: "List cleared", deletedAlert: "Data deleted",
      settingsTitle: "Settings", security: "Security", appLock: "App lock", appLockSub: "A passcode will be requested every time you open the app", lockOff: "Off", lockOnSuffix: "on", pinName: "Numeric code (PIN)", pinDesc: "4-digit number", patternName: "Pattern", patternDesc: "Connect the dots", alphaName: "Letters + numbers", alphaDesc: "Password (at least 4 characters)", changeLockBtn: "Change lock", disableLockBtn: "Disable lock", dataSection: "Data", lockSetupTitle: "Set up lock", newSecretTitle: "Enter a new passcode", repeatSecretTitle: "Repeat it", repeatSecretSub: "Enter it again to confirm", verifySecretTitle: "Enter passcode", currentLockSub: "Enter your current lock code", lockDisabledToast: "Lock disabled", lockSavedToast: "Lock saved", mismatchToast: "Doesn't match, try again", wrongTryAgainToast: "Wrong, try again", patternMinError: "Connect at least 4 dots", alphaPlaceholder: "Password (letters + numbers)", confirmBtn: "Confirm", alphaMinError: "At least 4 characters needed", wrongCodeToast: "Wrong code", appLockedTitle: "App is locked", appLockedSub: "Enter passcode to continue", forgotLink: "Forgot passcode? Log out", forgotConfirm: "All data (list, budget, history, lock) will be deleted and you'll return to the start screen. Continue?", transferNote: "Tapping will copy the amount to the clipboard and open your bank app.", bankSelectAria: "Choose bank", emptyListToast: "List is empty — add items first" }
  };
  ...
  // ---- Platform detection: to send the user to the right app store ----
  function isIOS(){
  ...
  function getInstallUrl(bank){
    if(isAndroid()) return 'https://play.google.com/store/search?q=' + encodeURIComponent(bank.name) + '&c=apps';
    if(isIOS()) return 'https://apps.apple.com/search?term=' + encodeURIComponent(bank.name);
    return bank.url; // Desktop: no app store to send to, just open the website
  }
  ...
  function launchBankApp(bank){
    if(items.length === 0){ showToast(getI18n('emptyListToast')); return; }
    const total = currentTotal();
    ...
    // Ашық жаңа бет алдын ала дайындалады (попап-блокерге ілінбеу үшін
    // тікелей пайдаланушы басқан сәтте, кідіріссіз ашылады).
  ...
  $('countryModalCloseBtn').addEventListener('click', ()=>{
    $('country-modal').style.display = 'none';
  });
  ...
  function renderLockStatus(){
    ...
    if(lockConfig.enabled){
      const names = { pin: getI18n('pinName'), pattern: getI18n('patternName'), alpha: getI18n('alphaName') };
      badge.textContent = names[lockConfig.type] + ' ' + getI18n('lockOnSuffix');
      badge.removeAttribute('data-i18n');
      badge.classList.remove('off'); badge.classList.add('on');
      chooser.style.display = 'none';
      manageRow.style.display = 'block';
    } else {
      badge.textContent = getI18n('lockOff');
      badge.setAttribute('data-i18n', 'lockOff');
  ...
  $('lockDisableBtn').addEventListener('click', ()=>{
    openLockVerify(lockConfig.type, (ok)=>{
      if(ok){
        ...
        renderLockStatus();
        showToast(getI18n('lockDisabledToast'));
      }
    }, getI18n('currentLockSub'));
  });
  ...
  function openLockSetup(type, mode){
    ...
    renderLockEntry($('lockSetupBody'), type, {
      title: getI18n('newSecretTitle'),
      sub: '',
      onComplete: (value)=>{
        if(firstEntry === null){
          firstEntry = value;
          renderLockEntry($('lockSetupBody'), type, {
            title: getI18n('repeatSecretTitle'),
            sub: getI18n('repeatSecretSub'),
            onComplete: (value2)=>{
              if(value2 === firstEntry){
                sha256Hex(type+':'+value2).then(h=>{
                  ...
                  $('lock-setup-modal').classList.remove('open');
                  renderLockStatus();
                  showToast(getI18n('lockSavedToast'));
                });
              } else {
                showToast(getI18n('mismatchToast'));
                firstEntry = null;
  ...
  function openLockVerify(type, callback, subText){
    $('lock-setup-modal').classList.add('open');
    renderLockEntry($('lockSetupBody'), type, {
      title: getI18n('verifySecretTitle'),
      sub: subText || '',
      onComplete: (value)=>{
        sha256Hex(type+':'+value).then(h=>{
          const ok = h === lockConfig.hash;
          callback(ok);
          if(!ok){ showToast(getI18n('wrongTryAgainToast')); openLockVerify(type, callback, subText); }
        });
      }
    });
  }
  ...
  function buildPatternEntry(wrap, onComplete){
    ...
    const errEl = document.createElement('div');
    errEl.className = 'lock-entry-error';
    errEl.textContent = getI18n('patternMinError');
    wrap.appendChild(errEl);
  ...
  function buildAlphaEntry(wrap, onComplete){
    ...
    input.placeholder = getI18n('alphaPlaceholder');
    input.autocomplete = 'off';
    const btn = document.createElement('button');
    btn.type='button'; btn.className='alpha-submit-btn'; btn.textContent = getI18n('confirmBtn');
    ...
    function submit(){
      ...
      if(v.length < 4){
        errEl.textContent = getI18n('alphaMinError');
  ...
  function showAppLockScreen(onUnlocked){
    ...
    renderLockEntry(body, lockConfig.type, {
      title: getI18n('appLockedTitle'),
      sub: getI18n('appLockedSub'),
      onComplete: (value)=>{
        sha256Hex(lockConfig.type+':'+value).then(h=>{
          ...
          if(ok){
            ...
            showToast(getI18n('wrongCodeToast'));
    ...
    forgotBtn.textContent = getI18n('forgotLink');
    forgotBtn.addEventListener('click', ()=>{
      if(!confirm(getI18n('forgotConfirm'))) return;
      try{
        localStorage.removeItem('sk-items');
        localStorage.removeItem('sk-budget');
        localStorage.removeItem('sk-archive');
